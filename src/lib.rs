...
fn validate_envvar(settings: &Settings, env_vars: &[apicore::EnvVar]) -> Result<()> {
    ...
    if let Settings::ContainsAnyOf { ref envvars } = settings {
        return contains_any_of(envvars, &resource_env_var_names);
    }

    if let Settings::DoesNotContainAnyOf { ref envvars } = settings {
        return does_not_contain_any_of(envvars, &resource_env_var_names);
    }

    if let Settings::ContainsAllOf { ref envvars } = settings {
        return contains_all_of(envvars, &resource_env_var_names);
    }

    if let Settings::DoesNotContainAllOf { ref envvars } = settings {
        return does_not_contain_all_of(envvars, &resource_env_var_names);
    }
    Ok(())
}

fn add_context_to_error(err: Error, container_name: String) -> Error {
    anyhow!(
        "Container \"{}\" has invalid environment variables. {}",
        container_name,
        err
    )
}
...
fn validate_environment_variables(
        ...
        if let Some(envvar) = &container.env {
            results.push(
                validate_envvar(settings, envvar)
                    .map_err(|err| add_context_to_error(err, container.name.clone())),
            );
        }
    }
    ...
    // merge all error messages in to a single string
    let errors_string: Vec<String> = results
        .into_iter()
        .filter_map(|result| result.err())
        .map(|err| err.to_string())
        .collect();
    Err(anyhow!(errors_string.join("\n")))
}
    ...
    fn test_multiple_container_validation(
        ...
        assert!(
            error.to_string().contains(
                format!(
                    "Container \"test-container\" has invalid environment variables. {}",
                    error_message
                )
                .as_str()
            ),
        ...
        assert!(
            error.to_string().contains(
                format!(
                    "Container \"test-container2\" has invalid environment variables. {}",
                    error_message
                )
                .as_str()
            ),
