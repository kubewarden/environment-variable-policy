...
mod tests {
    ...
    #[rstest]
    #[case(vec!["a".to_owned()], true)]
    #[case(vec!["a".to_owned(),"b".to_owned()], true)]
    #[case(vec!["a".to_owned(),"b".to_owned(), "c".to_owned()], true)]
    #[case(vec!["c".to_owned()], false)]
    #[case(vec!["b".to_owned(), "c".to_owned()], true)]
    #[case(vec![ ], false)]
    fn test_container_any_of(#[case] envvar: Vec<String>, #[case] is_ok: bool) {
        let default_envvar = HashSet::from(["a".to_owned(), "b".to_owned()]);
        let resource_env_var_names: HashSet<String> = envvar.into_iter().collect();
    ...
    #[rstest]
    #[case(vec!["a".to_owned()], false)]
    #[case(vec!["a".to_owned(),"b".to_owned()], false)]
    #[case(vec!["a".to_owned(),"b".to_owned(),"c".to_owned()], false)]
    #[case(vec!["c".to_owned()], true)]
    #[case(vec!["b".to_owned(), "c".to_owned()], false)]
    #[case(vec![ ], true)]
    fn test_does_not_container_any_of(#[case] envvar: Vec<String>, #[case] is_ok: bool) {
        let default_envvar = HashSet::from(["a".to_owned(), "b".to_owned()]);
        let resource_env_var_names: HashSet<String> = envvar.into_iter().collect();
    ...
    #[case(vec![ "b".to_owned(), "c".to_owned()], false)]
    #[case(vec![ ], false)]
    fn test_container_all_of(#[case] envvar: Vec<String>, #[case] is_ok: bool) {
        let default_envvar = HashSet::from(["a".to_owned(), "b".to_owned()]);
        let resource_env_var_names: HashSet<String> = envvar.into_iter().collect();
    ...
    #[rstest]
    #[case(vec!["a".to_owned()], true)]
    #[case(vec!["b".to_owned()], true)]
    #[case(vec!["a".to_owned(),"b".to_owned()], false)]
    #[case(vec!["a".to_owned(),"b".to_owned(),"c".to_owned()], false)]
    #[case(vec!["c".to_owned()], true)]
    #[case(vec!["b".to_owned(),"c".to_owned()], true)]
    #[case(vec![ ], true)]
    fn test_does_not_container_all_of(#[case] envvar: Vec<String>, #[case] is_ok: bool) {
        let default_envvar = HashSet::from(["a".to_owned(), "b".to_owned()]);
        let resource_env_var_names: HashSet<String> = envvar.into_iter().collect();
